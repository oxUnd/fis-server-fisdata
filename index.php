...
function fis_debug_render_smarty($tpl = null, $data = array()) {
    ...
    if (!$tpl) {
        ...
        $len = count($split);
        $namespace = $split[0] !== "" ? $split[0] : $split[1];
        ...
        $path_id = $namespace . ":" . substr($path, strpos($path, $namespace . '/') + strlen($namespace) + 1);
        $tpl = $root . 'template' . $path;
        $map = json_decode(file_get_contents($root . 'config/' . $namespace . '-map.json'), true);
        foreach ($map['res'] as $key => $value) {
            if($key == $path_id){
                $uri = $value["uri"] == '/' ? substr($value["uri"], 1) : $value["uri"];
                $tpl = $root . $uri;
                break;
            }            
        }
